...
NormBsDf = pd.read_csv('BootStrapSE.Norm.tsv', sep='\t', index_col=0)
...
NormAvgLen = AvgLen/AvgLen.sum()
NormPcDf = AvgPcDf.div(AvgPcDf.sum(axis=1), axis=0)
NormPcDf.to_csv('NormPcDf.tsv', sep='\t', header=True, index=True, index_label='Id')
NormPcScStatSigDf = pd.DataFrame(dtype=np.float64)
ColLs = list(NormPcDf.columns)

for Indx, row in NormPcDf.iterrows():
    for Col in ColLs:
        NormPcScStatSigDf.at[Indx, Col] = weightstats._zstat_generic(row[Col], NormAvgLen[Col], NormBsDf.at[Indx, f"SE_{Col}".replace('_Subs','')], alternative='two-sided',diff=0)[-1]/2

NormPcScStatSigDf.to_csv('NormScStatSigDf.tsv', sep='\t', header=True, index=True, index_label='Id')
